...
const PassengerProfile = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
        // Gọi API cập nhật
        const updatedUser = await mockService.updateProfile(formData.id, {
            fullName: formData.fullName,
            gender: formData.gender,
            avatar: formData.avatar
        });

        // Cập nhật lại localStorage để các trang khác nhận diện ngay
    ...
    <div className="max-w-md mx-auto md:max-w-2xl min-h-screen bg-gray-50 pb-20 font-sans">
      ...
      <div className="p-4">
        ...
        <form onSubmit={handleSave} className="space-y-5">
            ...
            {/* Giới tính (Editable) */}
            <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                 <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Giới tính</label>
                 <div className="flex gap-4">
                    <label className={`flex-1 py-3 rounded-xl border cursor-pointer text-center text-sm font-bold transition-all ${formData.gender === 'male' ? 'border-blue-500 bg-blue-50 text-blue-600' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
                        <input type="radio" name="gender" value="male" checked={formData.gender === 'male'} onChange={handleChange} className="hidden" />
                        Nam 👨
                    </label>
                    <label className={`flex-1 py-3 rounded-xl border cursor-pointer text-center text-sm font-bold transition-all ${formData.gender === 'female' ? 'border-pink-500 bg-pink-50 text-pink-600' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
            ...
            <div className="bg-gray-100 p-4 rounded-2xl border border-gray-200 opacity-80">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <p className="text-xs text-gray-500 font-medium">Thông tin định danh (Không thể sửa)</p>
                </div>
