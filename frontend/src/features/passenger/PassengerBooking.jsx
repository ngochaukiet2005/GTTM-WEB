...
const PassengerBooking = () => {
  ...
  const calculateAvailableSlots = () => {
    ...
    const cleanTripDate = new Date(tripDate.getFullYear(), tripDate.getMonth(), tripDate.getDate());
  ...
  const handleVerifyTicket = async (e) => {
    ...
    try {
      ...
      const tripDateTime = new Date(`${ticketForm.tripDate}T${ticketForm.departTime}`);
      ...
      await mockService.verifyTicket(ticketForm);
  ...
  const handleBooking = async () => {
    ...
    try {
      await mockService.createTrip({
        passengerId: "u1",
        tripCode: ticketForm.tripCode, 
        from: pickup,
        to: destination,
        distance: "3.5 km", 
        price: "Miễn phí",
        pickupTime: bookingTime, 
        tripDate: ticketForm.tripDate, 
        isFree: true 
      });
      
      // 👇 THÔNG BÁO XỊN: Popup Thành công thay cho Modal tự code
      Swal.fire({
        title: 'Đặt xe thành công!',
        html: `
          <div class="text-left text-sm text-gray-600 space-y-2">
            <p><b>Giờ đón:</b> ${bookingTime} - Ngày ${ticketForm.tripDate}</p>
            <p><b>Điểm đón:</b> ${pickup.address}</p>
            <p><b>Tài xế:</b> Đang điều phối...</p>
            <p class="text-green-600 font-bold mt-2">✨ Dịch vụ miễn phí cho hành khách!</p>
          </div>
        `,
        icon: 'success',
        confirmButtonText: 'Xem Lịch Sử Chuyến Đi',
        confirmButtonColor: '#2563eb', // Màu xanh blue-600
    ...
    <div className="relative h-screen w-full overflow-hidden font-sans bg-gray-50">
        ...
        <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-lg animate-fade-in-down max-h-[95vh] overflow-y-auto">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-bold text-blue-700">Xác Thực Vé Nhà Xe</h2>
              <p className="text-sm text-gray-500">Vui lòng điền đầy đủ thông tin vé để được xác nhận.</p>
            </div>
            
            <form onSubmit={handleVerifyTicket} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">Mã chuyến (*)</label>
                  <input required type="text" placeholder="VX001" className="w-full p-2 border rounded-lg uppercase"
                    value={ticketForm.tripCode} onChange={e => setTicketForm({...ticketForm, tripCode: e.target.value})} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">Email đặt vé (*)</label>
                  <input required type="email" placeholder="khach@gmail.com" className="w-full p-2 border rounded-lg"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="text" className="w-full p-2 border rounded-lg"
                    value={ticketForm.fullName} onChange={e => setTicketForm({...ticketForm, fullName: e.target.value})} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">Số điện thoại (*)</label>
                  <input required type="tel" className="w-full p-2 border rounded-lg" placeholder="090..."
                    value={ticketForm.phone} onChange={e => setTicketForm({...ticketForm, phone: e.target.value})} />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">Ngày khởi hành (*)</label>
                  <input required type="date" className="w-full p-2 border rounded-lg"
                    value={ticketForm.tripDate} onChange={e => setTicketForm({...ticketForm, tripDate: e.target.value})} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">Giờ xe chạy (*)</label>
                  <input required type="time" className="w-full p-2 border rounded-lg"
                    value={ticketForm.departTime} onChange={e => setTicketForm({...ticketForm, departTime: e.target.value})} />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                 <div>
                    <label className="block text-xs font-bold text-gray-600 mb-1">Điểm đón (Vé)</label>
                    <select className="w-full p-2 border rounded-lg bg-gray-50"
                      value={ticketForm.pickup} onChange={e => setTicketForm({...ticketForm, pickup: e.target.value})}>
                      {BUS_STATIONS.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                 </div>
                 <div>
                    <label className="block text-xs font-bold text-gray-600 mb-1">Điểm đến (Vé)</label>
                    <select className="w-full p-2 border rounded-lg bg-gray-50"
      ...
      {isVerified && (
        <>
            ...
            <button onClick={handleGetLocation} className="bg-white p-4 rounded-full shadow-xl text-gray-600 hover:text-blue-600">
            ...
            <div className="bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl p-6 border border-white/20 animate-fade-in-down">
                ...
                <div className={`relative z-10 flex items-center gap-4 mb-4 ${!pickup ? 'opacity-50' : 'opacity-100'}`}>
                    ...
                    <p className="text-sm font-bold text-gray-800 truncate">{pickup ? pickup.address : "..."}</p>
                    </div>
                </div>
                <div className={`relative z-10 flex items-center gap-4 ${!destination ? 'opacity-50' : 'opacity-100'}`}>
                    ...
                    <p className="text-sm font-bold text-gray-800 truncate">{destination ? destination.address : "..."}</p>
                    </div>
                </div>
                <button onClick={() => setIsGoingToStation(!isGoingToStation)} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white p-2 rounded-full shadow-md z-20">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" /></svg>
                </button>
                </div>
                ...
                <div className="mb-4">
                    ...
                    <div className="w-full p-3 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm font-bold text-center">
                        Không còn chuyến xe nào khả dụng trước giờ xe chạy ({ticketForm.departTime})
                    </div>
                    )}
                </div>
                ...
                <button 
                    ...
                    disabled={!selectedPoint || isBooking || availableSlots.length === 0}
